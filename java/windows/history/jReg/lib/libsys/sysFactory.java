...
//  System.out.println("win32] "+com.ms.dll.DllLib.getLastWin32Error());
    ...
    public void jSetPrivileges(String sePrivilege) throws Exception {
       try {
         int                         hToken[]  = {0};
         lib.libshellx.TOKEN_PRIVILEGES tp        = new lib.libshellx.TOKEN_PRIVILEGES();
         if (LookupPrivilegeValue(null, sePrivilege, tp.Privileges.Luid)) {
            ...
            if (OpenProcessToken (GetCurrentProcess(), wint.TOKEN_ADJUST_PRIVILEGES | wint.TOKEN_QUERY, hToken)) {
               ...
               if (AdjustTokenPrivileges (hToken[0], false, tp, 0, null, 0)) {
                  ...
                  new Exception("Fault: Privliges locked");
            }else
               new Exception("Fault: Privliges locked");
         }else
            new Exception("Fault: Privliges locked");
          ...
          eet =             "";
          eet = eet + "<es> : "+ e.toString()+"\n";
          std.errHandler(std.LOAD_EXCEPTION, "jSetPrivileges()", eet);
       }
    }
