...
/**
     ...
     cryptData = c.crypt(enc);
//   lib.libio.ioFactory      iw = new lib.libio.ioFactory(new File("jInstaller.ant"), cryptData, "Cp1252");
     lib.libio.ioFactory      iw = new lib.libio.ioFactory(new File("jInstaller.ant"), cryptData, "write");
                   iw.write();
                   iw.close();
**/
     lib.libio.ioFactory      ir = new lib.libio.ioFactory(new File("jInstaller.ant"), null, "read");
     String rslt = ir.read();
                   ir.close();

     String[] slip          = ns.Split(c.decrypt(rslt), ",");
//     String[] slip          = ns.Split(rslt, ",");
  ...
  try {
     ...
     if(regAction != null &&
        ...
        if (regAction.equalsIgnoreCase("create") &&
            values != null) {
           RegistryKey regKey = Registry.LOCAL_MACHINE.createSubKey(subKey);
           for (int j = 0; j < values.length; j++) {
              regKey.setValue(ns.Split(values[j], ";")[j-j],ns.Split(values[j], ";")[j-j+1]);
           }
        }else
           if (regAction.equalsIgnoreCase("delete")) {
              Registry.LOCAL_MACHINE.deleteSubKeyTree(subKey);
           }
     }else
...
public int objRegister(int modAction) {
  ...
  try {
     ...
     if(modAction != -1) {
      ...
      String eet        = null;
      eet =             "";
      eet = eet + "<es> : "+ e.toString() + "[win32.dll]:" + DllLib.getLastWin32Error()+"\n";
...
public String dialogOpen(String message) {
  try {
     ...
     if(message != null) {
        lib.shellx.BROWSEINFO bi = new lib.shellx.BROWSEINFO();
        if (message != null) {
  ...
  try {
     if(exePath != null &&
        ...
        if(!file.isDirectory()) {
           ...
           lib.shell.IShellLinkA  psl = (lib.shell.IShellLinkA)new lib.shell.ShellLinkA();
           lib.shell.IPersistFile ppf = (lib.shell.IPersistFile)psl;
